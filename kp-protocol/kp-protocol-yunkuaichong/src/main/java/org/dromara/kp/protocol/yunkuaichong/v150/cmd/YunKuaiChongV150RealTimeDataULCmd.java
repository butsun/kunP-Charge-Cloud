...
import org.dromara.kp.protocol.domain.dto.ChargingProgressProto;
import org.dromara.kp.protocol.domain.dto.GunRunStatusProto;
...
import static org.dromara.kp.protocol.domain.dto.GunRunStatusProto.GunRunStatus;
...
public class YunKuaiChongV150RealTimeDataULCmd extends YunKuaiChongUplinkCmdExe {
    ...
    public void execute(TcpSession tcpSession, YunKuaiChongUplinkMessage yunKuaiChongUplinkMessage, ProtocolContext ctx) {
        ...
        // 抢状态
        GunRunStatusProto.GunRunStatus gunRunStatus = parseGunRunStatus(gunStatus, gunInsert, tradeNo);
        GunRunStatusProto gunRunStatusProto = GunRunStatusProto.builder()
            .ts(ts)
            .pileCode(pileCode)
            .gunCode(gunCode)
            .gunRunStatus(gunRunStatus)
...
//        tcpSession.getForwarder().sendMessage(gunRunStatusMessage);
        ...
        if (StringUtils.isNotBlank(tradeNo)) {
            ...
            // 充电进度
            ChargingProgressProto chargingProgressProto = ChargingProgressProto.builder()
                .ts(ts)
                .pileCode(pileCode)
                .gunCode(gunCode)
                .tradeNo(tradeNo)
                .outputVoltage(outputVoltage.toPlainString())
                .outputCurrent(outputCurrent.toPlainString())
                .soc(soc)
